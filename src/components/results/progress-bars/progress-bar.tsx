...
export function ProgressBar({ children, progress, className }: Props) {
  return (
    <div
      className={
        className +
        " bg-base-950/60 relative w-full items-center overflow-hidden rounded-xl"
      }
    >
      ...
      {progress > 0 && (
        <div
          className="bg-base-100 absolute top-0 left-0 z-20 h-full overflow-hidden"
          style={{ width: Math.round(progress * 100).toString() + "%" }}
        >
          <div className="text-base-950 top-0 z-50 ml-4 flex h-full flex-col justify-center text-lg font-bold">
            {children}
          </div>
        </div>
      )}
